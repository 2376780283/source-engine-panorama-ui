...
inline void UnregisterForUnhandledEvents( void *pEventHandler ) { if ( UIEngine() ) { UIEngine()->UnregisterForUnhandledEvents( pEventHandler ); } }
...
// Split this out so we can template specialize other types in leaf code by:
// 1) inlcuding parseuieventparam.h
// 2) declaring our own extra specializations
// 3) including uievent.h which will "skip" this include due to include guard
#include "panorama/parseuieventparam.h"

// Use this macro to register an Enum as parseable for events via its underlying integer type.
#define REGISTER_PANORAMA_EVENT_ENUM( EnumType ) \
	namespace panorama \
	{ \
		template <> \
		inline bool ParseUIEventParam< EnumType >( EnumType *pOut, IUIPanel *pPanel, const char *pchEvent, const char **pchNextParam ) \
		{ \
			typedef std::underlying_type< EnumType >::type EnumUnderlyingType; \
			return ParseUIEventParam< EnumUnderlyingType >( ( EnumUnderlyingType * )pOut, pPanel, pchEvent, pchNextParam ); \
		} \
	}

// Use this macro to register an enum as parseable using the given function. The parse function should have the signature:
//   bool ParseFunction( const char *pszIn, EnumType *pOut )
#define REGISTER_PANORAMA_EVENT_ENUM_PARSE_FUNCTION( EnumType, EnumParseFunction ) \
	namespace panorama \
	{ \
		template <> \
		inline bool ParseUIEventParam< EnumType >( EnumType *pOut, IUIPanel *pPanel, const char *pchEvent, const char **pchNextParam ) \
		{ \
			...
			if ( !ParseUIEventParamHelper( bufValue, pchEvent, pchNextParam ) ) \
				return false; \
			\
			return EnumParseFunction( ( const char * )bufValue.Base(), pOut ); \
		} \
	}
...
void EscapeUIEventParam( CUtlString *pstrEscaped, const char *pchParam );
void UnescapeUIEventParam( char *pchParam );
//-----------------------------------------------------------------------------
// Purpose: Helpers to create documentation for an event
//-----------------------------------------------------------------------------
template < typename T >
const char *GetUIEventParamTypeName()
{
	// Fallback for non-specialized types. Compiler dependent.
	return typeid( T ).name();
}

template <> inline const char *GetUIEventParamTypeName< const char * >()	{ return "string"; }
template <> inline const char *GetUIEventParamTypeName< uint8 >()			{ return "uint8"; }
template <> inline const char *GetUIEventParamTypeName< uint16 >()			{ return "uint16"; }
template <> inline const char *GetUIEventParamTypeName< uint32 >()			{ return "uint32"; }
template <> inline const char *GetUIEventParamTypeName< uint64 >()			{ return "uint64"; }
template <> inline const char *GetUIEventParamTypeName< int32 >()			{ return "int32"; }
template <> inline const char *GetUIEventParamTypeName< int64 >()			{ return "int64"; }
template <> inline const char *GetUIEventParamTypeName< float >()			{ return "float"; }
template <> inline const char *GetUIEventParamTypeName< bool >()			{ return "bool"; }
template <> inline const char *GetUIEventParamTypeName< IUIEvent * >()		{ return "event"; }
template <> inline const char *GetUIEventParamTypeName< panorama::EPanelEventSource_t >()	{ return "paneleventsource"; }
template <> inline const char *GetUIEventParamTypeName< panorama::ScrollBehavior_t >()		{ return "scrollbehavior"; }
...
template <> void PanoramaTypeToV8Param< panorama::CStyleSymbol >( panorama::CStyleSymbol &pIn, v8::Handle<v8::Value> *pValueOut );
...
void SetPanelEventJSArg( const IUIPanel *pTarget, v8::Handle<v8::Value> *pValueOut );
...
void PanoramaTypeToV8Param( T * pIn, v8::Handle<v8::Value> *pValueOut )
{
	...
	else if ( panorama_is_base_of< IUIWindow, T >::value )
	{
		IUIWindow *pPanel = (IUIWindow*)pIn;
		return PanoramaPanelTypeToV8Param( pPanel, pValueOut );
	}
	...
	else
	{
		*pValueOut = v8::Null( GetV8Isolate() );
		AssertMsg( false, "PanoramaTypeToV8Param not implemented for pointer type" );
	}
}

template < typename T >
void PanoramaTypeToV8Param( panorama::CPanelPtr<T> &pIn, v8::Handle<v8::Value> *pValueOut )
{
	// Turn into a pointer and handle above
	PanoramaTypeToV8Param( pIn.Get(), pValueOut );
}
...
#define FORWARD_DECLARE_PANORAMA_EVENT_PTR( _className ) \
	namespace panorama { \
	template <> inline void PanoramaTypeToV8Param<const _className>( const _className * pIn, v8::Handle<v8::Value> *pValueOut ) \
	{ \
		*pValueOut = v8::Null( GetV8Isolate() ); \
		AssertMsg( false, "PanoramaTypeToV8Param not implemented for pointer type" ); \
	} \
	template <> inline void PanoramaTypeToV8Param<_className>( _className * pIn, v8::Handle<v8::Value> *pValueOut ) \
	{ \
		*pValueOut = v8::Null( GetV8Isolate() ); \
		AssertMsg( false, "PanoramaTypeToV8Param not implemented for pointer type" ); \
	} }
...
template <typename T> void FreeConvertedParam(T* pOut) {  }
template <> void FreeConvertedParam< const char *>( const char ** pszOut );
...
namespace UIEvent
{
	...
	inline CUtlString FormatEventArgs( const char *pchArgNames )
	{
		return FormatEventArgsCore( NULL, 0, pchArgNames );
	}

	template < class T >
	bool DecodeEvent( const panorama::IUIEvent *pEvent )
	{
		return ( pEvent && ( T::GetEventType() == pEvent->GetEventType() ) );
	}
}
...
#define DECLARE_PANORAMA_EVENT0( name ) \
	...
	public: \
		...
		static bool DecodeEvent( const panorama::IUIEvent *pEvent ) { return panorama::UIEvent::DecodeEvent< name >( pEvent ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs( pchArgNames ); } \
	}; \
...
namespace UIEvent
{
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		const char *arrArgTypes[] =
		{
			GetUIEventParamTypeName< param1 >()
		};

		return FormatEventArgsCore( arrArgTypes, V_ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1 ) 
	{ 
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;

		const EventType *pTypedEvent = assert_cast< const EventType* >( pEvent );
		*p1 = pTypedEvent->GetParam1();
		return true;
	}
}
...
#define DECLARE_PANORAMA_EVENT1( name, param1 ) \
	...
	public: \
		...
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent1< param1 >, param1 >( pEvent, p1 ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1 >( pchArgNames ); } \
	}; \
...
class  name \
	...
	public: \
		...
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIPanelEvent1< param1 >, param1 >( pEvent, p1 ); } \
		static panorama::IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEventCreate, const char **pchEventEnd ) { return panorama::UIEvent::CreateEventFromString< name, param1 >( pPanel, pchEventCreate, pchEventEnd ); } \
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1 >( pchArgNames ); } \
	}; \
...
namespace UIEvent
{
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		...
		{
			GetUIEventParamTypeName< param1 >(),
			GetUIEventParamTypeName< param2 >()
		};

		return FormatEventArgsCore( arrArgTypes, V_ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1, typename param2 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2 )
	{
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;
...
#define DECLARE_PANORAMA_EVENT2( name, param1, param2 ) \
	...
	public: \
		...
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent2< param1, param2 >, param1, param2 >( pEvent, p1, p2 ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2 >( pchArgNames ); } \
	}; \
...
namespace UIEvent
{
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		const char *arrArgTypes[] =
		{
			GetUIEventParamTypeName< param1 >(),
			GetUIEventParamTypeName< param2 >(),
			GetUIEventParamTypeName< param3 >()
		};

		return FormatEventArgsCore( arrArgTypes, V_ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1, typename param2, typename param3 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3 )
	{
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;
...
class  name \
	...
	public: \
		...
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent3< param1, param2, param3 >, param1, param2, param3 >( pEvent, p1, p2, p3 ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3 >( pchArgNames ); } \
	}; \
...
namespace UIEvent
{
	...
	{
		param1 pr1;
		if ( !ParseUIEventParam< param1 >( &pr1, pPanel, pchEvent, &pchEvent ) )
			return NULL;

		param2 pr2;
		if ( !ParseUIEventParam< param2 >( &pr2, pPanel, pchEvent, &pchEvent ) )
			return NULL;

		param3 pr3;
		if ( !ParseUIEventParam< param3 >( &pr3, pPanel, pchEvent, &pchEvent ) )
			return NULL;

		param4 pr4;
		if ( !ParseUIEventParam< param4 >( &pr4, pPanel, pchEvent, &pchEvent ) )
			return NULL;
		...
		*pchEventEnd = pchEvent;
		IUIEvent *pEvent = T::MakeEvent( pPanel ? pPanel->ClientPtr() : NULL, pr1, pr2, pr3, pr4 );
		UIEventFree( pr1 );
		UIEventFree( pr2 );
		UIEventFree( pr3 );
		UIEventFree( pr4 );
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		const char *arrArgTypes[] =
		{
			...
			GetUIEventParamTypeName< param2 >(),
			GetUIEventParamTypeName< param3 >(),
			GetUIEventParamTypeName< param4 >()
		};

		return FormatEventArgsCore( arrArgTypes, V_ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1, typename param2, typename param3, typename param4 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *pr1, param2 *pr2, param3 *pr3, param4 *pr4 )
	{
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;

		const EventType *pTypedEvent = assert_cast< const EventType* >( pEvent );
		*pr1 = pTypedEvent->GetParam1();
		*pr2 = pTypedEvent->GetParam2();
		*pr3 = pTypedEvent->GetParam3();
		*pr4 = pTypedEvent->GetParam4();
		return true;
	}
}
...
#define DECLARE_PANORAMA_EVENT4( name, param1, param2, param3, param4 ) \
	...
	public: \
		...
		static panorama::CPanoramaSymbol GetEventType() { Assert( symbol.IsValid() ); return symbol; } \
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4 ) { return new panorama::CUIEvent4< param1, param2, param3, param4 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4 ); } \
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3, param4 *pr4 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent4< param1, param2, param3, param4 >, param1, param2, param3, param4 >( pEvent, p1, p2, p3, pr4 ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4 >( pchArgNames ); } \
	}; \
...
#define DECLARE_PANEL_EVENT4( name, param1, param2, param3, param4 ) \
	...
	public: \
		...
		static panorama::CPanoramaSymbol GetEventType() { Assert( symbol.IsValid() ); return symbol; } \
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4 ) { return new panorama::CUIPanelEvent4< param1, param2, param3, param4 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4 ); } \
		static panorama::IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEventCreate, const char **pchEventEnd ) { return panorama::UIEvent::CreateEventFromString< name, param1, param2, param3, param4 >( pPanel, pchEventCreate, pchEventEnd ); } \
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4 >( pchArgNames ); } \
	}; \
...
namespace UIEvent
{
	...
	IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEvent, const char **pchEventEnd )
	{
		...
		*pchEventEnd = pchEvent;
		...
		IUIEvent *pEvent = T::MakeEvent( pPanel ? pPanel->ClientPtr() : NULL, p1, p2, p3, pr4, p5 );
		UIEventFree( p1 );
		UIEventFree( p2 );
		UIEventFree( p3 );
		UIEventFree( pr4 );
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		const char *arrArgTypes[] =
		{
			...
			GetUIEventParamTypeName< param3 >(),
			GetUIEventParamTypeName< param4 >(),
			GetUIEventParamTypeName< param5 >()
		};

		return FormatEventArgsCore( arrArgTypes, V_ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1, typename param2, typename param3, typename param4, typename param5 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3, param4 *pr4, param5 *p5 )
	{
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;
...
#define DECLARE_PANORAMA_EVENT5( name, param1, param2, param3, param4, param5 ) \
	...
	public: \
		...
		static panorama::CPanoramaSymbol GetEventType() { Assert( symbol.IsValid() ); return symbol; } \
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4, param5 p5 ) { return new panorama::CUIEvent5< param1, param2, param3, param4, param5 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4, p5 ); } \
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3, param4 *pr4, param5 *p5 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent5< param1, param2, param3, param4, param5 >, param1, param2, param3, param4, param5 >( pEvent, p1, p2, p3, pr4, p5 ); } \
		...
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4, param5 >( pchArgNames ); } \
	}; \
...
#define DECLARE_PANEL_EVENT5( name, param1, param2, param3, param4, param5 ) \
	...
	public: \
		...
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4, param5 p5 ) { return new panorama::CUIPanelEvent5< param1, param2, param3, param4, param5 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4, p5 ); } \
		static panorama::IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEventCreate, const char **pchEventEnd ) { return panorama::UIEvent::CreateEventFromString< name, param1, param2, param3, param4, param5 >( pPanel, pchEventCreate, pchEventEnd ); } \
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4, param5 >( pchArgNames ); } \
	}; \
...
//
// Events with 6 params
//
namespace UIEvent
{
	template < class T, typename param1, typename param2, typename param3, typename param4, typename param5, typename param6 >
	IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEvent, const char **pchEventEnd )
	{
		param1 p1;
		if ( !ParseUIEventParam< param1 >( &p1, pPanel, pchEvent, &pchEvent ) )
		...
		param6 p6;
		if ( !ParseUIEventParam< param6 >( &p6, pPanel, pchEvent, &pchEvent ) )
			return NULL;
		...
		*pchEventEnd = pchEvent;
		...
		IUIEvent *pEvent = T::MakeEvent( pPanel ? pPanel->ClientPtr() : NULL, p1, p2, p3, pr4, p5, p6 );
		UIEventFree( p1 );
		UIEventFree( p2 );
		UIEventFree( p3 );
		UIEventFree( pr4 );
	...
	CUtlString FormatEventArgs( const char *pchArgNames )
	{
		const char *arrArgTypes[] =
		{
			...
			GetUIEventParamTypeName< param4 >(),
			GetUIEventParamTypeName< param5 >(),
			GetUIEventParamTypeName< param6 >()
		};

		return FormatEventArgsCore( arrArgTypes, ARRAYSIZE( arrArgTypes ), pchArgNames );
	}

	template < class T, typename EventType, typename param1, typename param2, typename param3, typename param4, typename param5, typename param6 >
	bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3, param4 *pr4, param5 *p5, param6 *p6 )
	{
		if ( !pEvent || ( T::GetEventType() != pEvent->GetEventType() ) )
			return false;
		...
		*pr4 = pTypedEvent->GetParam4();
		*p5 = pTypedEvent->GetParam5();
		*p6 = pTypedEvent->GetParam6();
		return true;
	}
}

#define DECLARE_PANORAMA_EVENT6( name, param1, param2, param3, param4, param5, param6 ) \
	class  name \
	{ \
	public: \
		typedef param1 TypenameParam1; typedef param2 TypenameParam2; typedef param3 TypenameParam3; typedef param4 TypenameParam4; typedef param5 TypenameParam5; typedef param6 TypenameParam6; static const int cParams = 6; static const bool bPanelEvent = false; static panorama::CPanoramaSymbol symbol; static const char *pchEvent;\
		static panorama::CPanoramaSymbol GetEventType() { Assert( symbol.IsValid() ); return symbol; } \
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4, param5 p5, param6 p6 ) { return new panorama::CUIEvent6< param1, param2, param3, param4, param5, param6 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4, p5, p6 ); } \
		static bool DecodeEvent( const panorama::IUIEvent *pEvent, param1 *p1, param2 *p2, param3 *p3, param4 *pr4, param5 *p5, param6 *p6 ) { return panorama::UIEvent::DecodeEvent< name, panorama::CUIEvent6< param1, param2, param3, param4, param5, param6 >, param1, param2, param3, param4, param5, param6 >( pEvent, p1, p2, p3, pr4, p5, p6 ); } \
		static panorama::IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEventCreate, const char **pchEventEnd ) { return panorama::UIEvent::CreateEventFromString< name, param1, param2, param3, param4, param5, param6 >( pPanel, pchEventCreate, pchEventEnd ); } \
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4, param5, param6 >( pchArgNames ); } \
	}; \
	template< class T, class U > void RegisterEventHandler( const name &t, T *pPanel, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pPanel, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterEventHandler( const name &t, T *pPanel, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pPanel, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void RegisterEventHandlerOnPanel( const name &t, panorama::IUIPanel *pPanel, T *pHandler, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pHandler, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterEventHandlerOnPanel( const name &t, panorama::IUIPanel *pPanel, T *pHandler, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pHandler, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void RegisterForUnhandledEvent( const name &t, T *pObj, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterForUnhandledEvent( t.GetEventType(), UtlMakeDelegate( pObj, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterForUnhandledEvent( const name &t, T *pObj, bool (U::*memberfunc)( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterForUnhandledEvent( t.GetEventType(), UtlMakeDelegate( pObj, memberfunc ).GetAbstractDelegate() ); } \
	template< class PanelType > void RegisterEventHandlerOnPanelType( const name &t, bool( PanelType::*memberfunc )( param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterPanelTypeEventHandler( t.GetEventType(), PanelType::GetPanelSymbol(), UtlMakeDelegate( (PanelType*)NULL, memberfunc ).GetAbstractDelegate() ); }


#define DECLARE_PANEL_EVENT6( name, param1, param2, param3, param4, param5, param6 ) \
	class  name \
	{ \
	public: \
		typedef param1 TypenameParam1; typedef param2 TypenameParam2; typedef param3 TypenameParam3; typedef param4 TypenameParam4; typedef param5 TypenameParam5; typedef param6 TypenameParam6; static const int cParams = 6; static const bool bPanelEvent = true; static panorama::CPanoramaSymbol symbol; static const char *pchEvent;\
		static panorama::CPanoramaSymbol GetEventType() { return symbol; } \
		static panorama::IUIEvent *MakeEvent( const panorama::IUIPanelClient *pTarget, param1 p1, param2 p2, param3 p3, param4 pr4, param5 p5, param6 p6 ) { return new panorama::CUIPanelEvent6< param1, param2, param3, param4, param5, param6 >( symbol, pTarget ? pTarget->UIPanel() : NULL, p1, p2, p3, pr4, p5, p6 ); } \
		static panorama::IUIEvent *CreateEventFromString( panorama::IUIPanel *pPanel, const char *pchEventCreate, const char **pchEventEnd ) { return panorama::UIEvent::CreateEventFromString< name, param1, param2, param3, param4, param5, param6 >( pPanel, pchEventCreate, pchEventEnd ); } \
		static CUtlString FormatEventArgs( const char *pchArgNames ) { return panorama::UIEvent::FormatEventArgs< param1, param2, param3, param4, param5, param6 >( pchArgNames ); } \
	}; \
	template< class T, class U > void RegisterEventHandler( const name &t, T *pPanel, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pPanel, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterEventHandler( const name &t, T *pPanel, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pPanel, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void RegisterEventHandlerOnPanel( const name &t, panorama::IUIPanel *pPanel, T *pHandler, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pHandler, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterEventHandlerOnPanel( const name &t, panorama::IUIPanel *pPanel, T *pHandler, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterEventHandler( t.GetEventType(), pPanel, UtlMakeDelegate( pHandler, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void RegisterForUnhandledEvent( const name &t, T *pObj, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterForUnhandledEvent( t.GetEventType(), UtlMakeDelegate( pObj, memberfunc ).GetAbstractDelegate() ); } \
	template< class T, class U > void UnregisterForUnhandledEvent( const name &t, T *pObj, bool (U::*memberfunc)( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->UnregisterForUnhandledEvent( t.GetEventType(), UtlMakeDelegate( pObj, memberfunc ).GetAbstractDelegate() ); } \
	template< class PanelType > void RegisterEventHandlerOnPanelType( const name &t, bool( PanelType::*memberfunc )( const panorama::CPanelPtr< panorama::IUIPanel > &, param1, param2, param3, param4, param5, param6 ) ) { panorama::UIEngine()->RegisterPanelTypeEventHandler( t.GetEventType(), PanelType::GetPanelSymbol(), UtlMakeDelegate( (PanelType*)NULL, memberfunc ).GetAbstractDelegate() ); }

//
// Events declared in javascript
//
namespace UIEvent
{
	IUIEvent *JSCreateEventFromString( panorama::CPanoramaSymbol symEvent, panorama::IUIPanel *pPanel, const CUtlVector< v8::Local< v8::Value > > &args );
	IUIEvent *JSCreatePanelEventFromString( panorama::CPanoramaSymbol symEvent, panorama::IUIPanel *pPanel, const CUtlVector< v8::Local< v8::Value > > &args );
	CUtlString JSFormatEventArgs( const char *pchArgNames );
}
...
//
#define DEFINE_PANORAMA_EVENT_DOC_F( name, args, description, flags ) \
	panorama::CPanoramaSymbol name::symbol; \
	const char *name::pchEvent = #name; \
	panorama::CAutoRegisterUIEvent< name, name::cParams > g_##name##_EventAutoRegister( #name, args, description, flags );

#define DEFINE_PANORAMA_EVENT_DOC( name, args, description ) \
	DEFINE_PANORAMA_EVENT_DOC_F( name, args, description, panorama::k_eEventDocFlagNone )

#define DEFINE_PANORAMA_EVENT( name ) \
	DEFINE_PANORAMA_EVENT_DOC( name, "", "" )
...
{
public:
	CAutoRegisterUIEvent( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags )
	{
		RegisterUIEvent( &T::symbol, T::pchEvent, T::cParams, T::bPanelEvent, T::CreateEventFromString, T::FormatEventArgs, NULL, NULL, NULL, pchDocumentationArgs, pchDocumentationDescription, eDocFlags );
	}
};
...
public:
	CAutoRegisterUIEvent( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags )
	{
		RegisterUIEvent( &T::symbol, T::pchEvent, T::cParams, T::bPanelEvent, T::CreateEventFromString, T::FormatEventArgs, T::MakeEvent, NULL, NULL, pchDocumentationArgs, pchDocumentationDescription, eDocFlags );
	}
};
...
{
public:
	CAutoRegisterUIEventWithParam1( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags )
	{
		RegisterUIEvent( &T::symbol, T::pchEvent, T::cParams, T::bPanelEvent, T::CreateEventFromString, T::FormatEventArgs, NULL, NULL, NULL, pchDocumentationArgs, pchDocumentationDescription, eDocFlags );
	}
};
...
public:
	CAutoRegisterUIEventWithParam1( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags )
	{
		RegisterUIEvent( &T::symbol, T::pchEvent, T::cParams, T::bPanelEvent, T::CreateEventFromString, T::FormatEventArgs, NULL, NULL, T::MakeEvent, pchDocumentationArgs, pchDocumentationDescription, eDocFlags );
	}
};
...
public:
	CAutoRegisterUIEventWithParam1( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags )
	{
		RegisterUIEvent( &T::symbol, T::pchEvent, T::cParams, T::bPanelEvent, T::CreateEventFromString, T::FormatEventArgs, NULL, T::MakeEvent, NULL, pchDocumentationArgs, pchDocumentationDescription, eDocFlags );
	}
};
...
public:
	...
	CAutoRegisterUIEvent( const char *pch, const char *pchDocumentationArgs, const char *pchDocumentationDescription, EEventDocFlags eDocFlags ) : m_autoregister( pch, pchDocumentationArgs, pchDocumentationDescription, eDocFlags )
	{
	}
};
...
template < typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
class CUIEvent6 : public CUIEventBase
{
public:
	CUIEvent6( panorama::CPanoramaSymbol symEvent, const IUIPanel *pTargetPanel, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 ) : CUIEventBase( symEvent, pTargetPanel )
	{
		UIEventSet( &m_param1, param1 );
		UIEventSet( &m_param2, param2 );
		UIEventSet( &m_param3, param3 );
	...
	virtual ~CUIEvent6()
	{
		...
		UIEventFree( m_param4 );
		UIEventFree( m_param5 );
		UIEventFree( m_param6 );
	}

	virtual bool Dispatch( CUtlAbstractDelegate pFunc )
	{
		CUtlDelegate< bool( PARAM1_TYPE, PARAM2_TYPE, PARAM3_TYPE, PARAM4_TYPE, PARAM5_TYPE, PARAM6_TYPE ) > del;
		del.SetAbstractDelegate( pFunc );
		return del( m_param1, m_param2, m_param3, m_param4, m_param5, m_param6 );
	}

	virtual IUIEvent *Copy() const
	{
		return new CUIEvent6( GetEventType(), GetTargetPanel().Get(), m_param1, m_param2, m_param3, m_param4, m_param5, m_param6 );
	}
	...
	virtual void GetJavaScriptArgs( int *pCount, v8::Handle<v8::Value> **pArgs )
	{
		...
		*pArgs = new v8::Handle< v8::Value >[6];
		...
		PanoramaTypeToV8Param( m_param4, *pArgs + 3 );
		PanoramaTypeToV8Param( m_param5, *pArgs + 4 );
		PanoramaTypeToV8Param( m_param6, *pArgs + 5 );
	}
	...
	virtual void Validate( CValidator &validator, const tchar *pchName )
	{
		...
		UIEventValidate( validator, m_param4 );
		UIEventValidate( validator, m_param5 );
		UIEventValidate( validator, m_param6 );
	}
#endif
	...
	const PARAM4_TYPE &GetParam4() const { return m_param4; }
	const PARAM5_TYPE &GetParam5() const { return m_param5; }
	const PARAM6_TYPE &GetParam6() const { return m_param6; }
...
template < typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
class CUIPanelEvent6 : public CUIEventBase
{
public:
	CUIPanelEvent6( panorama::CPanoramaSymbol symEvent, const IUIPanel *pTargetPanel, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 ) : CUIEventBase( symEvent, pTargetPanel )
	{
		UIEventSet( &m_param1, param1 );
		UIEventSet( &m_param2, param2 );
		UIEventSet( &m_param3, param3 );
	...
	virtual bool Dispatch( CUtlAbstractDelegate pFunc )
	{
		CUtlDelegate< bool( const CPanelPtr< const IUIPanel > &, PARAM1_TYPE, PARAM2_TYPE, PARAM3_TYPE, PARAM4_TYPE, PARAM5_TYPE, PARAM6_TYPE ) > del;
		del.SetAbstractDelegate( pFunc );
		return del( GetTargetPanel(), m_param1, m_param2, m_param3, m_param4, m_param5, m_param6 );
	}

	virtual IUIEvent *Copy() const
	{
		return new CUIPanelEvent6( GetEventType(), GetTargetPanel().Get(), m_param1, m_param2, m_param3, m_param4, m_param5, m_param6 );
	}

	virtual void GetJavaScriptArgs( int *pCount, v8::Handle<v8::Value> **pArgs )
	{
		*pCount = 7;
		*pArgs = new v8::Handle< v8::Value >[7];
		SetPanelEventJSArg( GetTargetPanel().Get(), *pArgs );
		PanoramaTypeToV8Param( m_param1, *pArgs + 1 );
		PanoramaTypeToV8Param( m_param2, *pArgs + 2 );
		PanoramaTypeToV8Param( m_param3, *pArgs + 3 );
		PanoramaTypeToV8Param( m_param4, *pArgs + 4 );
		PanoramaTypeToV8Param( m_param5, *pArgs + 5 );
		PanoramaTypeToV8Param( m_param6, *pArgs + 6 );
	}
...
template < typename T, typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
bool DispatchEvent( T t, const IUIPanel *pTarget, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 )
{
	if ( !UIEngine()->BAnyHandlerRegisteredForEvent( T::symbol ) )
		return false;

	return UIEngine()->DispatchEvent( t.MakeEvent( pTarget ? pTarget->ClientPtr() : NULL, param1, param2, param3, param4, param5, param6 ) );
}
#endif
...
template < typename T, typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
bool DispatchEvent( T t, const IUIPanelClient *pTarget, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 )
{
	if ( !UIEngine()->BAnyHandlerRegisteredForEvent( T::symbol ) )
		return false;

	return UIEngine()->DispatchEvent( t.MakeEvent( pTarget, param1, param2, param3, param4, param5, param6 ) );
}
...
template < typename T, typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
void DispatchEventAsync( float flDelay, T t, const IUIPanel *pTarget, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 )
{
	if ( !UIEngine()->BAnyHandlerRegisteredForEvent( T::symbol ) )
		return;

	UIEngine()->DispatchEventAsync( flDelay, t.MakeEvent( pTarget ? pTarget->ClientPtr() : NULL, param1, param2, param3, param4, param5, param6 ) );
}
...
template < typename T, typename PARAM1_TYPE, typename PARAM2_TYPE, typename PARAM3_TYPE, typename PARAM4_TYPE, typename PARAM5_TYPE, typename PARAM6_TYPE >
void DispatchEventAsync( float flDelay, T t, const IUIPanelClient *pTarget, PARAM1_TYPE param1, PARAM2_TYPE param2, PARAM3_TYPE param3, PARAM4_TYPE param4, PARAM5_TYPE param5, PARAM6_TYPE param6 )
{
	if ( !UIEngine()->BAnyHandlerRegisteredForEvent( T::symbol ) )
		return;

	UIEngine()->DispatchEventAsync( flDelay, t.MakeEvent( pTarget, param1, param2, param3, param4, param5, param6 ) );
}
...
// Get additional template specializations for Steam
#include "../steamUI/tenfoot/tenfooteventhelpers.h"
#endif
