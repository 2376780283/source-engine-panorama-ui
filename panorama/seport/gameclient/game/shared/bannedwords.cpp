...
bool CBannedWordsDictionary::InitFromFile( char const *szFilename )
{
	...
	// Some systems (Linux/Mac) have 32 bit wchar_t, we implement conversion of the buffer here.
	if ( sizeof( wchar_t ) == 4 )
	{
		// Buffer size requires doubling space for the non-index part.
		const int nIndexSize = 0x10000 * sizeof( int32 );
		const int nBufSize = nIndexSize + (m_buf.Size() - nIndexSize) * 2;

		CUtlBuffer buf( nBufSize, nBufSize );

		int32* pIntSrc = ( int32* ) m_buf.Base();
		int32* pIntDst = ( int32* ) buf.Base();

		//Convert the index over, have to adjust the offsets.
		for ( int i = 0; i != 0x10000; ++i )
		{
			int32 offset = *pIntSrc++;

			offset = nIndexSize + ( offset - nIndexSize ) * 2;

			*pIntDst++ = offset;
		}

		//convert the main text over, 16 bit -> 32 bit.
		uint16* pShortSrc = (uint16*)pIntSrc;
		wchar_t* pWCharDst = (wchar_t*)pIntDst;

		const int nItems = ( m_buf.Size() - nIndexSize ) / 2;

		for ( int i = 0; i < nItems; ++i )
		{
			*pWCharDst++ = (wchar_t)*pShortSrc++;
		}
...
int CBannedWordsDictionary::CensorBannedWordsInplace( wchar_t *wsz ) const
{
	...
	for ( ; *wsz; ++ wsz )
	{
		...
		int32 nOffset = reinterpret_cast< int32 const * >( m_buf.Base() )[ static_cast<uint16>( wchThisLetter ) ];
